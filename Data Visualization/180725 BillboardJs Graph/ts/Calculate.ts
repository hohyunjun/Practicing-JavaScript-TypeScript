...
var resultValueQueue: Array<number> = [];
var defaultX = 120;
var defaultBarY = 380;
...
// 버튼 rect를 만드는 함수
function makeButtonNumbersRect(x: string,y: string,buttonfor: string):Element{
...
function makeButtonOperatorsRect(x: string, y: string, buttonfor: string):Element{
  var buttonRect = createSVG('rect');
  var buttonRectAttr = {
    'class': 'operators',
    'onclick': 'makeAppendFunction(' + buttonfor + ')()',
...
// 버튼 text를 만드는 함수
function makeButtonText(x: string, y: string, textfor: string):Element{
...
function makeGraphText(x: string, y: string, graphTextContent: string):Element{
...
function makeGraphLine(y: string):Element{
...
// Graph의 직사각형을 만드는 함수
function makeGraphRect(x: string):Element{
...
// Graph의 원을 만드는 함수
function makeGraphCircle(cx: string):Element{
...
function createSVG(tagname: string): Element{
  return document.createElementNS(svgNameSpace, tagname);
}

// SVG tag의 attribute를 설정해주는 함수
function setAttributes(element: Element, attrs: SVG):void{
...
function makeAppendFunction(char: string):Function{
  var char = char;
...
function changeMidValue(displayId:HTMLElement):void{
...
function modifyInnerText(elementId: string, innerValue: string):void{
    // svg 좌표 수정
    if(innerValue != ''){
...
function addInnerText(elementId: string, innerValue: string):void {
...
function appendQueue(resultValue: number):void{
...
function translateToHeight(resultValue: number): number{
  if(resultValue >= 110){
...
function translateToY(resultValue: number): number{
  if(resultValue >= 110){
...
// D3 Graph의 Container 정의. x축 y축 svg
...
function makeChartJsGraph(){
  var barCanvas:HTMLCanvasElement = <HTMLCanvasElement>document.getElementById("barChart")!;
